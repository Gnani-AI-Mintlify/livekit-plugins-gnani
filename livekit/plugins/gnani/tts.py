...
This module provides a TTS implementation that uses the Gnani Timbre API,
...
def _validate_voice_for_model(voice: str, model: str) -> None:
    ...
    if voice not in valid_voices:
        raise ValueError(f"Voice '{voice}' not supported for model '{model}'.")


def _validate_tts_config(
    *,
    voice: str,
    model: str,
    ...
    _validate_voice_for_model(voice, model)
...
class TTS(tts.TTS):
    ...
    Args:
        ...
        model: TTS model name (default: timbre-v2.0). Use timbre-v2.5 for the
            expanded 42-voice catalog with ``language`` control.
    ...
    def update_options(
        ...
        **kwargs: Any,
        ...
        new_voice = voice if voice is not None else self._opts.voice
        new_model = model if model is not None else self._opts.model
        new_language = language if language is not None else self._opts.language

        _validate_tts_config(voice=new_voice, model=new_model, language=new_language)
...
class _Pcm16Aligner:
    ...
    the next chunk so downstream int16 consumers never see odd-length buffers.
    """
...
def _append_pcm(aligner: _Pcm16Aligner, audio: bytes) -> bytes:
    return aligner.align(_strip_wav_header(audio))
...
def _emit_pcm(
    aligner: _Pcm16Aligner,
    output_emitter: tts.AudioEmitter,
    audio: bytes,
) -> None:
    aligned = _append_pcm(aligner, audio)
    if aligned:
        output_emitter.push(aligned)
...
def _finalize_pcm(
    ...
    """Drop any dangling byte at end of stream (incomplete 16-bit sample)."""
    aligner.reset()
                ...
                **_ws_header_kwargs(_build_headers(self._opts, request_id=api_request_id)),
                ...
                async for msg in ws:
                    if isinstance(msg, bytes):
                        _process_and_push(pusher, output_emitter, msg)
                        continue
